...
- `src/modules/leg/`: gleicher Aufbau wie `shoulder/`. Rahmen B Becken, U Rumpf (Kontext), F Femur, P Patella, T Tibia+Fibula, A Talus, C Fuß, F2…C2 anderes Bein (gespiegelt um die Beckenmitte oder eigene Pose je `other`). `kinematics.js`: Pose `{hipF,hipA,hipR,knee,tibR,valg,ankle,sub,wt,ground,tilt,other}`; bei Bodenkontakt steht der Fuß am festen Bodenpunkt (Druckmittelpunkt stetig Ferse ↔ Ballen), der Rumpf neigt sich für das Gleichgewicht (`hipF` = Oberschenkel zur Senkrechten, klinische Hüftbeugung = `hipF` + Neigung + `tilt`; Rumpfursprung dreht mit dem Becken, Neigung iterativ); Gelenkmomente aus Bodenreaktion/Segmentgewichten, Kraftverteilung je Gelenk ∝ PCSA·Hebel (Quadrizeps am Knie über die Patellarsehne; Hüftmuskeln mit seitengebundenem Wrapping um die Hüftkugel), PF-Kraft als Vektorsumme, TF-Kompression mit medial/lateraler Verteilung (Adduktionsmoment), VKB/HKB aus dem Schub entlang der Tibia, Achilles-/Hüftkraft, FADIR-Abstand, Tractus-Lage, Sprunggelenk-Impingements. `EXT.at` (`foot`/`body`) und `EXT.fixHip` für Lasten. Hebelarme prüfen: `node tests/tools/momentarms-leg.mjs`.
...
4. **Referenzlängen**: Muskeln relativ zur Neutralstellung; Bänder/Kapsel als normierte Spannung 0 … 12 % zwischen 85 % und 100 % ihrer Maximallänge im Bewegungsraum, mit anatomischen Ankern (`taut`, `endAt`). Wer in `computeReferenceLengths` eine Pose löst, muss am Ende **die Ruhestellung wieder herstellen**, bevor `REF.lbsDefl` gesetzt wird.
   Wrapping-Kugeln, die ein Gelenkzentrum umschließen, brauchen `side` (Seite der Umlenkung), sonst kippt der Hebelarm in tiefer Beugung (LESSONS 36).
...
Bein (75 kg, `BW` = 735 N): Stand PF ≈ 0, TF ≈ 0,9 × KG, medial 50–65 % · Kniebeuge parallel PF 2,5–4,5 × KG, Rumpf 30–60° · tief (140°) PF 4–7,5 × KG, Druck 4–10 MPa, Hinterhörner komprimiert, Glutaeus aktiv · Treppe einbeinig PF 2,5–4,5, Hüfte 2–7 × KG · Beinstrecker: VKB nahe Streckung > 150 N und > 3 × Wert bei 60°, jenseits 90° null · Tractus-Reibzone 10–35° · vorderes Sprunggelenk-Impingement < 3 mm bei 35° Dorsalextension, hinteres < 3 mm bei −45° · ATFL beim Supinationstrauma ≥ +8 % · Wadenheben Achilles 1–4 × KG · FADIR < 4 mm, Stand > 25 mm · Varus 8° > 70 % medial, Valgus 8° < 35 % · Kreuzheben unten: Glutaeus > 0,25, Iliopsoas 0 · Ausfallschritt ausbalanciert, Ballen des hinteren Fußes auf Bodenhöhe · alle Ansätze ≤ 4 mm am Knochen. Änderungen an Anatomie oder Kinematik immer gegen `npm test` prüfen und die Anker bei Bedarf bewusst nachziehen.

## Fahrplan

Engine/Modul-Schnittstelle (Registry, Umschalter, `MODULE`-Objekt: **fertig, v0.10**) → Bein-Modul (Hüfte–Knie–Sprunggelenk: **erste Fassung v0.11, Review v0.12** mit Becken/Rumpf, Kompartimenten, FADIR, zweitem Bein; Offenes siehe HANDOVER §8) → Rumpf-Modul (LWS–Becken–Hüfte) → HWS als Erweiterung der Schulter. Module nach kinetischen Ketten schneiden, nicht nach Einzelgelenken. Ein neues Modul: Ordner unter `src/modules/<id>/` mit `module.json` (Bauteile, Name, Alias), letzte Datei liefert `MODULE` nach dem Muster von `shoulder/module.js`, dann die ID an `registry.json` **anhängen**; der Umschalter erscheint automatisch.
