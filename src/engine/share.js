/* ===================== Geteilte Links: Zustand ⇄ URL-Parameter =====================
   ...
   lang · run (laufender Ablauf) · E P rot elbow pro (Pose) · load · speed · loop · migr gird frozen ·
...
const POSE_KEYS=[['E','E'],['P','P'],['IR','rot'],['elbow','elbow'],['pro','pro']];
...
function currentState(){
  ...
  if(patho.migr>0) add('migr',numStr(patho.migr)); if(patho.gird>0) add('gird',String(Math.round(patho.gird*100))); if(patho.frozen>0) add('frozen',String(Math.round(patho.frozen*100)));
...
function compactField(k,v){
  ...
  switch(k){
    ...
    case 'E': return 'E'+n(v); case 'P': return 'P'+n(v); case 'rot': return 'R'+n(v); case 'elbow': return 'B'+n(v); case 'pro': return 'F'+n(v);
    case 'speed': return 'S'+Math.round(+v*100); case 'loop': return 'Q0';
    case 'migr': return 'M'+Math.round(+v*10); case 'gird': return 'G'+n(v); case 'frozen': return 'Z'+n(v);
...
function compactDecode(tok){
  ...
  for(const f of tok.split('_')){ if(!f) continue; const k=f[0], v=f.slice(1);
    switch(k){
      ...
      case 'E': o.E=v; break; case 'P': o.P=v; break; case 'R': o.rot=v; break; case 'B': o.elbow=v; break; case 'F': o.pro=v; break;
      case 'S': o.speed=String(+v/100); break; case 'Q': o.loop=v; break;
      case 'M': o.migr=String(+v/10); break; case 'G': o.gird=v; break; case 'Z': o.frozen=v; break;
...
function applyURLState(){
  ...
  let v;
  const pa={}; if((v=num('migr'))!==null) pa.migr=clamp(v,0,6); if((v=num('gird'))!==null) pa.gird=clamp(v,0,100)/100; if((v=num('frozen'))!==null) pa.frozen=clamp(v,0,100)/100; if(Object.keys(pa).length) setPatho(pa);
...
Object.assign(window.Schulterlabor,{currentState,buildShareURL,applyURLState,URL_PARAMS,compactEncode,compactDecode});
