/* ===================== Schulter: Kameras, Detailansichten, Schichtstufen =====================
   ...
   detailDefs: die vier Detailfenster (id = Element-ID in page.html) mit Kamera-Fit je Pose und Anzeigetext (readout).
...
const detailDefs=[
  {id:'dv1',fov:26,level:2,readout:(M)=>fmt(M.ahd*10,1)+' mm',
  ...
  {id:'dv2',fov:26,level:4,clipX:0.55,readout:(M)=>t('ant')+' '+fmt(M.capAnt*100)+' % · '+t('post')+' '+fmt(M.capPost*100)+' %',
   fit(fr){ const t=fr.H.p.clone().add(V3(-1.2,0,0)); const d=V3(1,0.28,0.32).normalize(); return {t,pos:t.clone().addScaledVector(d,14)}; }},
  {id:'dv3',fov:26,level:2,readout:(M)=>t('kink')+' '+(M.lbsDeflRel>=0?'+':'')+fmt(M.lbsDeflRel)+'°',
  ...
  {id:'dv4',fov:28,level:0,readout:(M,rh)=>rh?t('scapUp')+fmt(rh.UR)+'° · '+t('tilt')+' '+fmt(rh.PT)+'°':null,
