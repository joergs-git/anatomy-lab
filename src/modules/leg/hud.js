...
const READOUT0={lean:0,hipClin:0,screw:-8,balanced:true,grf:0,ground:true,comZ:0,tilt:0,tibR:0,otherMode:1,otherGround:true};
...
function hudBodyHTML(ps,rh,M){
  return `${t('hip')} ${fmt(rh.hipClin)}° (${t('thigh')} ${fmt(ps.hipF)}°) · ${sgn(ps.hipA,'add','abd')} ${fmt(Math.abs(ps.hipA))}° · ${sgn(ps.hipR,'ER','IR')} ${fmt(Math.abs(ps.hipR))}°<br>`+
    `${t('knee')} ${fmt(ps.knee)}° · ${t('tibRot')} ${sgn((rh.tibR||ps.tibR)+rh.screw,'ER','IR')} ${fmt(Math.abs((rh.tibR||ps.tibR)+rh.screw))}° · ${sgn(ps.valg,'varus','valgus')} ${fmt(Math.abs(ps.valg))}°${Math.abs(ps.tilt)>=1?' · '+t('tiltS')+' '+sgn(ps.tilt,'tiltPost','tiltAnt')+' '+fmt(Math.abs(ps.tilt))+'°':''}<br>`+
...
function balanceReadoutHTML(ps,rh){
  ...
  return `${t('trunkLean')} ${fmt(rh.lean)}°${rh.balanced?'':' · '+t('unbalanced')} · ${t('hipClinL')} ${fmt(rh.hipClin)}°<br>`+
    ...
    `${t('screwL')} ${fmt(Math.abs(rh.screw))}° ${t('ER')}`+(Math.abs((rh.tibR||0)-ps.tibR)>=0.5?` · ${t('pronL')} ${fmt((rh.tibR||0)-ps.tibR)}° ${t('IR')}`:'')+`<br>${t('tiltL')} ${(ps.tilt>=0?'+':'')+fmt(ps.tilt)}° · ${t('otherL')}: ${t(['otAuto','otMirror','otLift','otRear','otFront'][rh.otherMode]||'otMirror')}`;
}
