# Übergabe an Claude Code – Stand 13. September 2026
...
### Stand Bein-Modul (13. September 2026, v0.12.0): Review Anatomie, Pathologien, Becken/Rumpf, anderes Bein

Anlass war eine fachliche Durchsicht (vollständige Anatomie? typische orthopädische Befunde erkennbar? kinetische Kette Fuß–Knie–Hüfte–Becken–Rumpf? Sehnen „in der Luft“? gespiegeltes Bein falsch?). Ergebnis:

- **Ansätze auf dem Knochen.** Alle 88 Ursprünge/Ansätze (ohne Nerv) liegen jetzt ≤ 4 mm an einem Knochenprimitiv ihres Rahmens; dafür wurden Tibiaplateau und Metaphyse unter die Kondylen gesetzt (Plateau y −3,2), Malleolen auf Achshöhe angehoben (Spitzen ≈ 1 cm unter der Achse), Würfelbein, Sustentaculum tali, Processus lateralis tali, Spinae (ASIS/AIIS/PSIS), unterer Schambeinast und linkes Sitzbein ergänzt und ~25 Landmarken verschoben (Gerdy, Pes anserinus, Peronei am Würfelbein, Tibialis posterior am Kahnbein/Sustentaculum, Bänder des Sprunggelenks). Der Test `Ansätze` (`tests/leg.test.mjs`) baut die Knochen über einen Aufzeichnungs-Kontext (`harness.recordBones()`) und misst analytisch den Abstand zu Ellipsoiden/Röhren – ohne Szene.
- **Sprunggelenkbänder neu verankert**: ATFL Fibulaspitze → Talushals lateral (max. bei Plantarflexion + Inversion), CFL → Calcaneus (max. bei Dorsalextension + Inversion), PTFL → Processus posterior (max. in Dorsalextension), Deltaband zweiteilig → Sustentaculum/Kahnbein (max. bei Eversion). Basisfaktoren je Band (`base` in `computeReferenceLengths`, Deltaband 0,90).
- **Kreuzbänder**: tibiale Insertionen auf das abgesenkte Plateau, femorale Ursprünge so gewählt, dass das AM-Bündel nahezu isometrisch bleibt (100 → 92 % bei 90°) und das PL-Bündel in Beugung entspannt (→ 71 %); der Rotations-/Valgus-Zuschlag zur VKB-Spannung wurde auf 0,35 %/° bzw. 0,25 %/° angehoben, damit Cutting > Stand bleibt.
- **Becken und Rumpf als Kette**: neue Pose `tilt` (Beckenkippung gegen den Rumpf, −20…25°, Kurz-Link `t`), neue Strukturen Erector spinae (lumbal), Rectus abdominis, Obliqui, Adductor longus/Pectineus, Piriformis/tiefe Außenrotatoren (Gruppe „Rumpf & Becken“, Schichtstufe 2). Vorwärtskippung verkürzt Iliopsoas und Rückenstrecker und dehnt Bauch und Hamstrings (unteres gekreuztes Syndrom), Rückwärtskippung umgekehrt; Presets Hohlkreuz, Flachrücken, Thomas-Test. Der Rumpfursprung (L5/S1) dreht jetzt mit dem Becken (vorher stand er fest, was bei Rumpfneigung Rumpfmuskeln scheinbar dehnte); die Neigung wird deshalb in drei Fixpunktschritten gelöst.
- **Hüftmuskeln in tiefer Beugung**: Glutaeus maximus und Iliopsoas kehrten rechnerisch ihr Vorzeichen um (gerade Linie durch das Gelenkzentrum; Glutaeus im Kreuzheben inaktiv, Iliopsoas als „Strecker“ zu 55 %). Lösung: seitengebundenes Wrapping (`wrapSphere(..., side)` in `engine/math.js`: die Umlenkung muss auf der angegebenen Seite der Kugel liegen, sonst wird der Pfad über einen Stützpunkt geführt) um eine Hüftkugel r 3,4 cm – Beuger vorn (`WRAP_HIP_ANT`), Glutaeus hinten-unten (`WRAP_HIP_POST`); unterer Glutaeus-Anteil über das Sitzbein/Lig. sacrotuberale geführt, Trochanter minor posteromedial gesetzt, Iliopsoas über die Eminentia iliopubica. Streck-Hebel des Glutaeus jetzt 4,2 (0°) → 5,5 (30°) → 3,8 (90°) → 3,0 cm (120°); Iliopsoas bleibt Beuger bis 120°. Diagnose: `node tests/tools/momentarms-leg.mjs`.
- **Hüftimpingement (FADIR)**: Abstand Kopf-Hals-Übergang (femurlokal, sphärisch platziert) ↔ vorderer-oberer Pfannenrand: neutral ≈ 36 mm, 90° Beugung ≈ 5 mm, FADIR ≈ 2 mm, Außenrotation frei. Monitor-Zeile „Hüfte (Kontaktkraft, Impingement)“ unter „Speziell“.
- **Kompartimente**: das Frontalebenen-Moment am Knie (Bodenreaktionslinie neben dem Kniezentrum, gegen die Neutralstellung `REF.dxKnee0`) verteilt die TF-Kompression medial/lateral mit Sättigung (`tanh`, ≈ 90 % bei 8 cm Hebel): Stand 55 % medial, Varus 8° ≈ 85 %, Valgus 8° ≈ 20 %, Trendelenburg 91 %, Knick-Senkfuß 13 %. Kette Fuß → Knie: Eversion bei Bodenkontakt koppelt 0,6°/° Tibia-Innenrotation (`pronationIR`), Preset „Knick-Senkfuß“.
- **Anderes Bein** (Pose `other`, Kurz-Link `o`, Segmentknöpfe): 0 automatisch (gespiegelt; ab 90 % Gewichtsanteil abgehoben), 1 gespiegelt, 2 abgehoben, 3 hinten am Boden, 4 vorn gestreckt. Der frühere Fehler „Bein spiegelt sich nach außen weg“ kam daher, dass um die feste Weltmitte gespiegelt wurde, während das Becken bei Bodenkontakt seitlich wandert – jetzt Spiegelung um die Beckenmitte. Modus 3 löst das hintere Bein als Zweiglieder-Kette mit fester Schrittlänge (`REAR_STRIDE` 80 cm hinter der Sohlenmitte, Ballen auf Bodenhöhe, Sohle ≈ 50°; unerreichbar → Fuß rutscht nach vorn, Hüftstreckung ≤ 25°), und das Gleichgewicht rechnet den zweiten Auflagepunkt anteilig ein (Ausfallschritt jetzt ausbalanciert mit 13° Rumpf statt „−60°, unbalanciert“). Bodenplatte bis unter den hinteren Fuß verlängert.
- **Druckmittelpunkt stetig**: Auflagepunkt wandert linear von Ferse (≥ 1,5 cm tiefer) über Sohlenmitte zum Ballen statt zu springen (vorher sprang das Kniemoment bei 1° Sprunggelenkänderung um 10 cm Hebel).
- **Übungen**: Kreuzheben 60 kg (Hüfthinge: Hüfte 34°/Knie 25°, Rumpf ergibt sich aus dem Gleichgewicht → Hüfte 94° klinisch, Glutaeus 0,37, Hamstrings 0,56, Iliopsoas 0). Brücke und Clamshell wurden wieder entfernt: Rückenlage/Seitlage lassen sich mit fester Schwerkraftrichtung nicht abbilden (die Brücke ergab 5 × KG patellofemoral). Split Squat startet tiefer (Hüfte 42°/Knie 50°), damit die Schrittlänge erreichbar bleibt.
- **Anker verschoben (bewusst)**: Hüftkontaktkraft Treppe 2–7 × KG (vorher 2–6): mit beteiligtem Glutaeus maximus steigt die Muskelsumme; das Preset ist ein statischer Extremfall mit 60° Rumpf, Bergmann in vivo ≈ 2,5–3,5 × KG.
- **Bekannt und offen**: Pistol-Squat PF 8,2 × KG (Literatur 4–6; PF/Quadrizeps-Verhältnis 1,5 bei 105° eher hoch, aber die vier Kniebeuge-Anker passen); Glutaeus medius sättigt im Trendelenburg (Anzeige > 100 % = Überforderung, gewollt); Syndesmose nicht modelliert (Fibula starr an der Tibia); Vorfuß/Zehen weiterhin starr; Piriformis kehrt seine Rotationswirkung in Beugung nicht um; Rectus femoris wird ab ≈ 110° Hüftbeugung rechnerisch Strecker (AIIS liegt dann unter dem Muskelweg; real ist er dort schlaff – für die Kraftverteilung ohne Folgen, weil das Knie ihn über die Patellarsehne führt); Quadratus lumborum, Glutaeus minimus, Tibialis-Nerv/Tarsaltunnel, Hallux/Windlass fehlen.
